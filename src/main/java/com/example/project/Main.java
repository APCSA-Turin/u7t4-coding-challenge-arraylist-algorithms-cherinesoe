   ...
   */
    ...
    public static ArrayList<String> parseSentence(String sentence){
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (spaceIdx != -1) {
                String add = sentence.substring(0, spaceIdx);
                parsed.add(add);
                sentence = sentence.substring(spaceIdx + 1);
                i = spaceIdx + 1;
            }
        }
        parsed.add(sentence);
   ...
   */
    public static ArrayList<String> moveBWords(ArrayList<String> wordList){
        ...
        for (int i = 0; i < wordList.size(); i++) {
            String first = wordList.get(i).substring(0, 1);
            if (first.equals("b") || first.equals("B")) {
    ...
    public static ArrayList<Integer> swapEnds(ArrayList<Integer> list){  
        if (list.size() < 1) {
            ...
            int first = list.set(0, list.get(list.size() - 1));
    ...
    public static ArrayList<Integer> fix34(ArrayList<Integer> list){
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) == 3) {
                for (int j = lastFixedIdx; j < list.size(); j++) {
                    if (list.get(j) == 4) {
